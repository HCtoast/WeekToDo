...
/**
 * 다른 블록 위에 얹힌 TODO를 얼마나 밀지 (px).
 *
 * 전폭으로 얹으면 아래 구글 일정이 통째로 사라진다.
 * 아래 블록의 왼쪽 색 띠가 드러날 만큼만 민다.
 */
export const STACKED_TODO_OFFSET_PX = 10

/**
 * 겹친 TODO 묶음 카드에서 한 줄의 높이 (px).
 *
 * 체크박스(11px)와 12px 글자가 한 줄에 들어가는 최소 높이다. 줄이 낮으면 더 많이
 * 담을 수 있지만 글자가 눌려 읽기 어려워진다. 시작 시각이 이 높이만큼(≈18분) 벌어지지
 * 않은 항목들은 카드 안에서 아래로 밀려난다.
 */
export const TODO_CLUSTER_ROW_PX = 16
